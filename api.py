...
@app.route("/session/requere", methods=["POST"])
def requerirsessoes():
    requerimento = request.get_json()
...
def deletarSessao():
    ...
    if decisao:
        cursor.execute(f'drop table "{idsessao}"')
        conn.commit()
        cursor.execute(f"DELETE FROM hosts WHERE id_table = '{idsessao}'")
        conn.commit()
...
def verificarTableID(tableID=str()):
    if func.invalidValue(tableID,10,10):
        return False
    cursor.execute("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'DECJJEEQQJ')")
    return cursor.fetchone()[0]
